...
#include <SPI.h>
#include "SPIFFS.h"          // <-- ADD: for file system
...
// --- File Transfer Function (USB serial) ---
// <-- ADD: sends file over USB serial
...
void setup() {
  ...
  // <-- ADD: Print ready message for Python script
...
void loop() {
  // <-- ADD: Check for serial command (file transfer)
  if (Serial.available()) {
      String command = Serial.readStringUntil('\n');
      command.trim();
      if (command == "SEND_FILE") {
          sendFile("/data.txt");   // send the logged data file
          // Return to main loop; display may be temporarily interrupted
      }
      if (command == "CLEAR_FILE") {
        if (SPIFFS.remove("/data.txt")) {
          Serial.println("!FILE DELETED");
        } else {
          Serial.println("!ERROR: File not found or could not be deleted");
      }
}
  }
  ...
  switch (currentState) {
    ...
    case SENDING_CMD:{
      dataLoggedForTest = false; // RESET FLAG
    ...
    case RECEIVED:{
       ...
       // Write to file only if this is the first result of this test
       if (!dataLoggedForTest) {
         File dataFile = SPIFFS.open("/data.txt", FILE_APPEND);
         if (dataFile) {
           dataFile.print(millis());
           dataFile.print(": ");
           dataFile.println(incomingData.payload);
           dataFile.close();
           Serial.println("Data saved to /data.txt");
           dataLoggedForTest = true;        // prevent further writes
         } else {
